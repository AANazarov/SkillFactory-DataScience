...
### ***Аннотация проекта***
В данном проекте выполняется первичная обработка и разведочный анализ базы данных резюме из Headhunter.
***
...
* `Project-1.ipynb` - основной рабочий jupiter-ноутбук
* каталог `data` - содержит исходные данные
* каталог `plotly` - для импорта графиков в формате *.html

Файл dst-3.0_16_1_hh_database.csv, имеющий размер более 100MB, в каталоге `data` отсутствует - после импорта проекта в локальный репозиторий его необходимо скачать и поместить в каталог `data` вручную.
***  

### ***Рекомендации по импорту проекта***
Данный проект является частью удаленного репозитория **SkillFactory-DataScience**.<p>
